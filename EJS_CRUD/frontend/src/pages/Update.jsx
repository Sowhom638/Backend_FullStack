...
useEffect(() => {
  setloading(true);
  axios.patch(`http://localhost:4000/movie/edit/${id}`)
  .then((response) => {
    setloading(false);
  ...
  const handleSubmit = () => {
    ...
    setloading(true);
    axios.patch(`http://localhost:4000/movie/edit/${id}`,data)
    .then(() => {
      setloading(true);
      navigate('/');
    })
  ...
  return (
    <>
      ...
      {loading ? <Spinner /> : <div className="bg-slate-600 w-full m-4 p-4 rounded-2xl">
        ...
        <button className="rounded-2xl p-2 w-20 h-10 bg-teal-300 mt-4" onClick={handleSubmit}>Update</button>
        </div>}
    </>
  )
}
